...
dapps.map((dapp) => {
    dappsMd += `| <img src="${path.resolve(__dirname, '../dapps/img')}/${dapp.logo}" /> | ${dapp.title} | ${dapp.description} | <img src="${path.resolve(__dirname, '../dapps/img')}/${dapp.preview}" /> | ${dapp.link} | ${dapp.categoriy} | ${dapp.gaTag} | \n`
})
...
partners.map((partner) => {
    partner.children.map((item) => {
        partnersMd += `| <img src="${path.resolve(__dirname, '../partners/img')}/${item.logo}" /> | ${item.title} | ${item.description} | ${item.link} | ${partner.type} | ${item.gaTag} | \n`
    })
})
...
homePartners.map((partners) => {
    partners.map((partner) => {
        homePartnersMd += `| <img src="${path.resolve(__dirname, '../home-partners/img')}/${partner.logo}" /> | ${partner.link} | ${partner.gaTag} | \n`
    })
})
